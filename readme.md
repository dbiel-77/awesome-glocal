# GLOCAL Community Scripts  
![GLOCAL Logo](logo-dark.png)

[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)
[![Contributions Welcome](https://img.shields.io/badge/contributions-welcome-brightgreen.svg)](contributing.md)
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](#)
[![GLOCAL](https://img.shields.io/badge/powered%20by-YouCount-yellow)](volunteer.youcount.ca)
[![Maintenance](https://img.shields.io/badge/maintained-yes-green.svg)](#)
[![Open Source](https://img.shields.io/badge/open%20source-yes-lightgrey.svg)](#)

> A curated list of useful, accessible scripts built by members of the GLOCAL community.  
> Focused on simplifying workflows, improving usability, and supporting users across different technical skill levels.

---

## Contents

- [About](#about)
- [Categories](#categories)
- [General Tools](#general-tools)
- [Contributing](#contributing)

---

## About

This repository highlights practical, real-world scripts developed through GLOCAL initiatives, with an emphasis on:

- Accessibility and ease of use  
- Automation of repetitive workflows  
- Lightweight, deployable tools  
- Cross-skill usability (beginner → advanced)

---

## Categories

This collection is in its early stages.

Categories will evolve as contributions are added. Early contributors will directly shape how this repository is structured.

---

## General Tools

### PDF Converter

- [File Converter](https://github.com/Ayodeji0001/File-Converter)  
  *Tags:* `Web` `PDF` `Converter` `Beginner-Friendly`  
  Convert PDFs to Text, DOCX, Markdown, HTML, SVG, or Images (PNG/JPEG) via a simple web interface.  
  **Author:** Ayodeji ([@ajyodeji0001](https://github.com/ajyodeji0001))

---

## Contributing

Contributions are open.

- Submit your contribution through our new volunteer platform:  
  https://volunteer.youcount.ca  
- Use the official task link:  
  https://volunteer.youcount.ca/tasks/38465705-d70f-4d32-b238-37233731ae31  
- Follow the task instructions to start developing your script.

---

## Contribution Guidelines

Please review the [contributing guidelines](contributing.md) before submitting.